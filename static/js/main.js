document.addEventListener("DOMContentLoaded", () => {
    ...
    // Video slider functionality - completely rewritten for better performance
    const setupVideoSliders = () => {
        ...
        sliders.forEach((slider) => {
            ...
            // Make video grid scrollable horizontally on mobile
            videoGrid.style.display = "flex"
            videoGrid.style.overflowX = "auto"
            videoGrid.style.scrollSnapType = "x mandatory"
            videoGrid.style.webkitOverflowScrolling = "touch"
            videoGrid.style.scrollbarWidth = "none"
            videoGrid.style.msOverflowStyle = "none"

            // Hide scrollbar
            videoGrid.addEventListener(
                "scroll",
                () => {
                    // This is just to ensure smooth scrolling on iOS
                },
                { passive: true },
            )
            ...
            videoCards.forEach((card) => {
                ...
                wrapper.style.width = "180px" // Base width
                wrapper.style.marginRight = "15px"
                wrapper.style.scrollSnapAlign = "start"

                // Use media queries for responsive sizing
                if (window.innerWidth >= 768) {
                    wrapper.style.width = "220px" // Larger on desktop
                }

                // Replace the card with the wrapper containing the card
            ...
            if (prevArrow && nextArrow) {
                prevArrow.addEventListener("click", (e) => {
                    e.preventDefault()
                    const scrollAmount = videoGrid.clientWidth * 0.8
                ...
                videoGrid.addEventListener("scroll", () => {
                    ...
                    const isAtEnd = videoGrid.scrollLeft + videoGrid.clientWidth >= videoGrid.scrollWidth - 10
                ...
                // Initial arrow state
                prevArrow.style.opacity = "0.3" // Start with prev arrow faded
            }
        })
    }
    ...
    window.addEventListener("resize", setupVideoSliders)
